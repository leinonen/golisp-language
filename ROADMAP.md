...
- [x] **Auto-promote across concrete numeric types in arithmetic** — `(/ concrete-int 2.0)` / `(+ int-var float-var)` / `(< i x)` mixing a concrete `int` and a concrete `float64` operand no longer produce a Go "mismatched types"/"truncated to integer" error; the int operand is wrapped in `float64(...)` (Clojure promotes silently). `emitArith`/`emitBinOp` (`emit_expr.go`) call `mixesIntFloat` and, when a form has both a concrete int and a concrete float operand, `emitPromotedOperand` wraps each int. Operand kinds come from `numericKind`/`callNumericKind`, backed by a scalar numeric env: `localNumeric` (typed params via `registerVarType`, typed/inferred `let` & `loop` bindings, scoped like `localTypes`/`localAny`) and `globalNumeric` (typed top-level `def`s, pre-pass). Sources recognised: numeric literals, typed vars/globals, `(int …)`/`(float64 …)`/`(as T …)` casts, `math/*` (→ float64), nested arithmetic, and user fns/methods with a numeric `-> T`. Untyped int constants stay flexible (no spurious promotion), pure-int and pure-float forms stay native, and `mod` is skipped (Go `%` is integer-only). The separate `any`-seam coercion (helper path) is unchanged — it fires first.
...
**Status**: shipped — 12a/12b/12d/12e/12f + stdlib qualifier loading complete; the
ADR-015 "use Go libraries easily" story (signature loading, variadic spread,
multi-return diagnostics, stdlib coverage, arg coercion, editor support) is
delivered. 12c is deferred (see its note).

- [x] **12a. Signature loader** — `LoadGoPackages(dir, paths)` (`internal/transpiler/gopkg.go`) loads exported package-level function signatures (per-param Go types, variadic flag, single return type) of each declared `(:import …)` package via `go/packages` (`NeedName | NeedTypes`), keyed by call-site qualifier. The compiler runs it once per dir build after `ResolveDeps` (`compileDir`), attaching the result to the `DeclSet` (`SetGoPackages`/`GoImportPaths`); the emitter reads it in the pre-pass. **Degrades gracefully**: an unloadable/offline package is simply absent and its calls emit untyped — never a build failure — and the load is skipped entirely when no external packages are imported (zero cost for the common stdlib-only case). First consumer: **completes 12b's deferred validation** — spreading `& xs` into an imported function whose loaded signature is *not* variadic is now a position-tagged `.glsp` error (verified end-to-end against `github.com/google/uuid`) instead of a raw Go error. Builds gain a `golang.org/x/tools` dependency (build-time only; not in generated binaries).
  - [x] **Stdlib qualifier loading** — `GoImportPaths` now also loads referenced **stdlib** packages, not just declared `(:import …)`. `CollectDecls` records every `pkg/fn` call-site qualifier from the **token stream** (robust and complete — string/comment contents are separate token kinds, so no fragile AST walk), and `GoImportPaths` resolves each through `stdlibByQualifier`, skipping built-in namespaces (`json`/`re`/`ctx`/`log`/`http`), glisp `:require` module aliases, and ambiguous qualifiers (`rand`). So interop diagnostics/typing reach stdlib calls too — e.g. `(os/create …)` as a single value now gets the multi-return `if-err` diagnostic with no `:import` (verified end-to-end). ~90–140 ms to load a typical 8-package set, and only when qualifiers are referenced (no module context → silent degradation). This is the prerequisite 12c/12d needed for stdlib return/param types.
- [x] **12b. Variadic auto-spread** — `(pkg/fn a b & xs)` → `pkg.Fn(a, b, xs...)`; `& sym` reuses the existing `fn`-param `& rest` spelling (it lexes as a symbol, so no parser change). `spreadArgs` (`emit_expr.go`) splits a call's args around the marker; the general call path emits the spread value with a trailing `...`. Closes `go-interop-exploration §3.7` — the last documented reason a wrapper module needs hand-written Go (`(fmt/printf fmt & args)` → `fmt.Printf(fmt, args...)`; `[]any` spreads straight into a Go `...any` param). Spreading into a **user** fn validates against its known signature (a fixed-arity callee → `"is not variadic"` error; too few leading args → arity error); spreading into an **external** Go call trusts the marker until 12a's loader can validate it. Misplaced/duplicate markers are position-tagged errors. (Subsumes the Phase 11a "eliminate hand-written Go" item for the variadic case.)
- [~] **12c. Typed returns from interop calls** — **deferred** (recommended won't-do as originally specced). The premise was that a `(pkg/fn …)` return would flow into dot-free method dispatch / `(:field …)` without an `(as T …)`. But that machinery (ADR-013) resolves only on glisp-**declared** structs/interfaces, whereas interop calls return **external** Go types whose fields/methods the loader doesn't capture (it loads package-level funcs, not type members). So `(:field (pkg/fn …))` / `(method (pkg/fn …))` can't resolve without loading external type members *and* extending dispatch to non-glisp types — a large change that cuts against "Go's runtime model, undiluted" (ADR-012). Meanwhile the cases that *do* matter are already covered: a typed `let`/`def` binding of an interop call keeps its concrete Go type via `:=` inference, and `(.Method (pkg/fn …))` interop already composes (`(time.Now()).Year()`). Two narrow sub-cases remain *if a concrete need arises*: (a) interop calls returning concrete numeric types participating in int↔float auto-promotion (extend `callNumericKind` to consult loaded sigs), and (b) passing a concrete-int interop result to a `float64` stdlib param. Both are small, isolated follow-ups — not the broad feature. Revisit only with real-code evidence.
- [x] **12d. Coerce `any` args at all imported call sites** — a loaded function's real parameter types now thread into its argument hints in the general call path (`paramHintsFor` in `gopkg.go`, variadic-aware: trailing args get the element type, not the `[]T` slice), generalizing the `math/*`-only `stdlibNumericParams` table to every loaded package. `any` arguments coerce at any typed Go parameter — numeric via `numericCoercion` (`_glispToInt`/`_glispToFloat64`), **string via the forgiving `_glispToString`** (new branch in `emitExprWithHint`, matching the `(string x)` cast / `:- string` destructuring rather than a brittle `.(string)` assertion), and other concrete types via the existing assertion path. So `(strings/to-upper (first xs))` emits `strings.ToUpper(_glispToString(_glispFirst(xs)))` instead of a raw `cannot use … (any) as string` error (closes `go-interop-exploration §3.4`); concrete args are emitted unchanged (`exprIsAny` guards every coercion). Verified end-to-end against `strings`/`strconv`/`path` + `uuid`.
- [ ] **12e. Glisp-level interop diagnostics** — wrong arity, a wrong-typed argument, or a field/method that doesn't exist on the package's type surfaces as a position-tagged `.glsp` error at transpile time (ADR-011 rule 3), driven by the loaded signatures, instead of a raw `go/types`/`go build` error.
  - [x] **Multi-return interop calls** — an imported Go function whose loaded signature returns 2+ values (`pgx.Connect` → `(*pgx.Conn, error)`, `uuid.Parse` → `(uuid.UUID, error)`), used in single-value position, now gets the same `if-err` diagnostic as `multiReturnBuiltins` (closes `go-interop-exploration §3.5`). `goFunc.results` records every return type; `multiReturnCall` (`emit_arity.go`) consults the loaded index, so every existing single-value gate (fn/loop tails, `let`/`if-let`/`let-or`/`def` bindings) covers interop calls too. Verified end-to-end against `github.com/google/uuid` (external) and `os/create` (stdlib, via the stdlib qualifier loading above).
- [x] **12f. LSP completion/hover from loaded signatures** — hovering a `pkg/fn` interop symbol shows its real Go signature (`func strings.ToUpper(s string) string`), and completion after a `pkg/` qualifier offers that package's exported functions with their signatures (kebab-case prefixes match PascalCase names: `strings/to-up` → `ToUpper`). Exported `GoSignatures` API (`gopkg.go`: `LoadGoSignatures`/`Signature`/`Completions`, capturing `types.ObjectString` at load) wraps the same loader the transpiler uses; the server (`server.go`) memoizes it per directory + import-path set so editing doesn't re-shell the toolchain. Best-effort — no signatures load → normal local/builtin hover/completion unchanged. Satisfies the ADR-012 tooling-parity invariant. *(Package-level functions; struct fields/methods would follow the same path if needed.)*
