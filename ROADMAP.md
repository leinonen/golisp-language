...
- [ ] **Cross-file struct field access** — `(:field b)` on a variable typed as a struct defined in another file currently falls back to `_glispGet(b, "field")`, which returns `nil` for struct values at runtime. Fix: a first-pass that collects all `defstruct`/`definterface`/`defmethod`/`defn` declarations from every file in the directory before any file is emitted, making `e.structs` and `e.ifaces` globally populated for that build. Workaround today: define accessor functions in the same file as the struct (`defn book-id [b Book] -> string (:id b)`) and call them from other files.
- [ ] **Cross-file interface method dispatch** — `(method-name repo arg)` where `repo` is typed as an interface defined in another file is emitted as a plain function call `methodName(repo, arg)` rather than `repo.MethodName(arg)`, because `resolveMethodCall` only looks in `e.ifaces` for the current file. Workaround: explicit dot-method syntax `(.MethodName repo arg)`. Fix: same first-pass type collection as above.
- [ ] **Inline comments inside `let` bindings** — `;` comment after a binding expression causes a parse error ("expected symbol, got comment"). Comments are only valid between complete forms, not inside `[binding expr]` pairs. Workaround: place comments on the line above the binding.
...
- [ ] **Typed atoms** — atoms store `any`, so `(get (deref store) id)` returns `any` and requires `(as T v)` type-assertion on retrieval. A typed atom `(atom map[string]Book {})` would validate stored values and eliminate `as` noise. Workaround: accept scattered `(as Book entry)` assertions (safe when only one type is ever stored).
- [ ] **Atoms as struct fields** — `defstruct` fields require Go types; `(atom {})` is a runtime value, not a type, so `(defstruct Repo store (atom {}))` is invalid. Stateful structs that need their own atom must use a module-level `def`, making each struct a process singleton. Fix: allow an `atom` field type in defstruct (e.g. `store (atom map[string]any)`) that emits a Go struct with the inline atom struct embedded.
...
- [ ] **`(map f coll)` returns `[]any`, not a typed slice** — `_glispMap` always returns `[]any`, so `(map (fn [v] (as Book v)) entries)` can't satisfy a `[]Book` return type. Interface methods that logically return `[[]Book error]` must instead declare `[any error]`, losing type safety. Fix: emit a typed map loop (`var result []Book; for _, v := range coll { result = append(result, f(v).(Book)) }`) when the transform function has a concrete return type annotation, or provide a `(typed-map ^Book f coll)` variant.
