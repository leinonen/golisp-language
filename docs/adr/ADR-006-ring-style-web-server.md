...
The web layer uses the Ring convention: a handler is a function that takes a request map and returns a response map. Middleware is a function from handler to handler.
...
- **Composable** — middleware stacks are built by function composition: `(web/Wrap handler WrapLogging WrapJson WrapAuth)`
