...
var builtinDocs = map[string]BuiltinDoc{
	...
	"web/wrap-cors":      {Sig: "(web/wrap-cors handler)                    →  Handler", Doc: "Add permissive CORS headers to responses."},
	...
	"web/wrap-recover":   {Sig: "(web/wrap-recover handler)                 →  Handler", Doc: "Recover from panics and return a 500 response."},
	"web/wrap-timeout":   {Sig: "(web/wrap-timeout seconds)                 →  Middleware", Doc: "Abort the request with 503 if the handler takes longer than seconds."},
	"web/json-response":  {Sig: "(web/json-response status body)            →  Response", Doc: "Build a Ring-style response map with the given HTTP status and JSON body."},
	...
	"web/serve-files":    {Sig: "(web/serve-files prefix dir)               →  Handler", Doc: "Serve static files from dir under the URL prefix."},
