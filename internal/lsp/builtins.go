...
var builtinDocs = map[string]BuiltinDoc{
	...
	"web/Request":  {Sig: "type Request = map[string]any", Doc: "Ring-style request map. Keys: \"method\", \"path\", \"query\", \"headers\", \"body\"."},
	...
	"web/Routes":        {Sig: "(web/Routes & routes)                     →  Handler", Doc: "Combine route definitions into a single HTTP handler."},
	"web/GET":           {Sig: "(web/GET pattern handler)                 →  Route", Doc: "Define a GET route matching pattern, handled by handler."},
	...
	"web/WrapJson":      {Sig: "(web/WrapJson handler)                    →  Handler", Doc: "Parse request JSON body and store it in req[\"json-body\"] for the handler."},
	"web/WrapCors":      {Sig: "(web/WrapCors handler)                    →  Handler", Doc: "Add permissive CORS headers to responses."},
	"web/WrapAuth":      {Sig: "(web/WrapAuth handler)                    →  Handler", Doc: "Extract Bearer token from Authorization header and store in req[\"identity\"]."},
	"web/WrapLogging":   {Sig: "(web/WrapLogging handler)                 →  Handler", Doc: "Log each request method, path, and status code."},
	"web/WrapRecover":   {Sig: "(web/WrapRecover handler)                 →  Handler", Doc: "Recover from panics and return a 500 response."},
	"web/WrapTimeout":   {Sig: "(web/WrapTimeout seconds)                 →  Middleware", Doc: "Abort the request with 503 if the handler takes longer than seconds."},
	"web/JsonResponse":  {Sig: "(web/JsonResponse status body)            →  Response", Doc: "Build a Ring-style response map with the given HTTP status and JSON body."},
	"web/QueryParam":    {Sig: "(web/QueryParam req name)                 →  string", Doc: "Return the value of URL query parameter name from the request."},
	"web/PathParam":     {Sig: "(web/PathParam req name)                  →  string", Doc: "Return the value of URL path parameter name from the request."},
	"web/BodyMap":       {Sig: "(web/BodyMap req)                         →  map[string]any", Doc: "Return the JSON-decoded body as a plain map (not a Response — the body content itself)."},
	"web/Header":        {Sig: "(web/Header req name)                     →  string", Doc: "Return the value of HTTP header name from the request."},
	"web/ServeFiles":    {Sig: "(web/ServeFiles prefix dir)               →  Handler", Doc: "Serve static files from dir under the URL prefix."},
	"web/Serve":         {Sig: "(web/Serve addr handler)                  →  error", Doc: "Start an HTTP server on addr with handler. Blocks until an error occurs."},
	"web/ServeGraceful": {Sig: "(web/ServeGraceful addr handler)", Doc: "Start an HTTP server on addr; blocks and shuts down gracefully on SIGINT/SIGTERM."},
