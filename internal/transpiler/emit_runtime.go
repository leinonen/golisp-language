...
// Slices and maps are compared structurally by recursing through _glispEquals,
// so a nil slice equals an empty slice (filter/etc. return nil on empty input,
// while a [] literal is a non-nil []any{}) and element types may differ
// (a []int compares equal to an equal []any).
func _glispEquals(a, b any) bool {
	...
	if isSeq(ka) && isSeq(kb) {
		if va.Len() != vb.Len() {
			return false
		}
		for i := 0; i < va.Len(); i++ {
			if !_glispEquals(va.Index(i).Interface(), vb.Index(i).Interface()) {
				return false
			}
		}
		return true
	}
	if ka == reflect.Map && kb == reflect.Map {
		if va.Len() != vb.Len() {
			return false
		}
		for _, k := range va.MapKeys() {
			...
			if !_glispEquals(va.MapIndex(k).Interface(), bv.Interface()) {
				return false
			}
		}
		return true
