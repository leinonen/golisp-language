...
**`case` (alias for `switch`)**: `(case expr v1 b1 ... default?)` is the Clojure-style spelling of `switch` — the fallthrough is a trailing unpaired form instead of `:default`. It is **not** a distinct AST node: the parser leaves it as a plain `CallExpr`, and the transpiler rewrites it into a `SwitchExpr` via `caseCallToSwitch` at each emit position (`emitCallExpr` → value, `emitStmtNode` → statement, `emitReturnNode` → typed return), so it reuses all of `switch`'s emission (including clean typed returns — no `any` IIFE). This CallExpr-not-node design is deliberate: the formatter treats `case` as a Style-A pair-form (it's in `pairForms` alongside `assoc`/`cond->`), aligning clauses under the dispatch value, and round-trips it as `case`. Arity (`{2,-1}`) is gated by the central `builtinArity` table.
