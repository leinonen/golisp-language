...
- [ ] **12e. Glisp-level interop diagnostics** — wrong arity, a wrong-typed argument, or a field/method that doesn't exist on the package's type surfaces as a position-tagged `.glsp` error at transpile time (ADR-011 rule 3), driven by the loaded signatures, instead of a raw `go/types`/`go build` error.
  - [x] **Multi-return interop calls** — an imported Go function whose loaded signature returns 2+ values (`pgx.Connect` → `(*pgx.Conn, error)`, `uuid.Parse` → `(uuid.UUID, error)`), used in single-value position, now gets the same `if-err` diagnostic as `multiReturnBuiltins` (closes `go-interop-exploration §3.5`). `goFunc.results` records every return type; `multiReturnCall` (`emit_arity.go`) consults the loaded index, so every existing single-value gate (fn/loop tails, `let`/`if-let`/`let-or`/`def` bindings) covers interop calls too. Verified end-to-end against `github.com/google/uuid`. *(Declared-import scope, like 12a.)*
