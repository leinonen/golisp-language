...
var BuiltinDocs = map[string]BuiltinDoc{
	...
	"count":        {Sig: "(count coll)          →  int", Doc: "Number of elements in coll. Works on any-typed values."},
	"len":          {Sig: "(len coll)            →  int", Doc: "Alias for count. Number of elements in coll; works on any-typed values."},
	...
	"upper-case":   {Sig: "(upper-case s)            →  string", Doc: "Legacy — prefer (str/upper s). Return s with all letters uppercased."},
	"lower-case":   {Sig: "(lower-case s)            →  string", Doc: "Legacy — prefer (str/lower s). Return s with all letters lowercased."},
	"trim":         {Sig: "(trim s)                  →  string", Doc: "Legacy — prefer (str/trim s). Remove leading and trailing whitespace from s."},
	"starts-with?": {Sig: "(starts-with? s prefix)   →  bool", Doc: "Legacy — prefer (str/starts-with? s prefix). True when string s begins with prefix."},
	"ends-with?":   {Sig: "(ends-with? s suffix)     →  bool", Doc: "Legacy — prefer (str/ends-with? s suffix). True when string s ends with suffix."},
	"replace":      {Sig: "(replace s old new)       →  string", Doc: "Legacy — prefer (str/replace s old new). Replace all occurrences of old with new in s."},
	"split":        {Sig: "(split s sep)             →  []string", Doc: "Legacy — prefer (str/split s sep). Split s on sep and return the parts as a slice."},
	"join":         {Sig: "(join coll sep)           →  string", Doc: "Legacy — prefer (str/join sep coll) (note: sep first). Join the elements of coll into a string separated by sep."},
	...
	"defmacro":   {Sig: "(defmacro name [params...] body...)", Doc: "Define a compile-time macro. The body runs at macroexpansion time on the unevaluated argument forms (as data) and returns a form; use syntax-quote (`), unquote (~), unquote-splice (~@), and auto-gensym (name#). Inspect output with `glisp macroexpand`."},
	...
	"if":         {Sig: "(if cond then else?)", Doc: "Evaluate then when cond is truthy (only nil and false are falsy), else (or nil) otherwise."},
	...
	"some->":     {Sig: "(some-> x f1 f2 ...)   nil-safe thread-first", Doc: "Like -> but short-circuits to nil the moment any step's input is nil. Replaces nested nil-guards over optional chains (e.g. (some-> req (get \"user\") (get \"email\")))."},
	"some->>":    {Sig: "(some->> x f1 f2 ...)  nil-safe thread-last", Doc: "Like ->> but short-circuits to nil at the first nil step."},
	"cond->":     {Sig: "(cond-> x test1 form1 test2 form2 ...)", Doc: "Conditional thread-first: thread x through each form whose paired test is truthy, accumulating; falsy tests leave the value unchanged. Every clause is checked (unlike cond)."},
	"cond->>":    {Sig: "(cond->> x test1 form1 test2 form2 ...)", Doc: "Conditional thread-last: like cond-> but threads the value as the last argument of each chosen form."},
	"GET":        {Sig: "(GET pattern [params] body...)", Doc: "Routing DSL: a GET route whose body is the handler. Names in [params] bind matching path params (web/path-param); req is in scope. Expands to (web/get pattern (fn [req] ...)). Requires (:import [golisp/web])."},
	"POST":       {Sig: "(POST pattern [params] body...)", Doc: "Routing DSL: a POST route. See GET."},
	"PUT":        {Sig: "(PUT pattern [params] body...)", Doc: "Routing DSL: a PUT route. See GET."},
	"DELETE":     {Sig: "(DELETE pattern [params] body...)", Doc: "Routing DSL: a DELETE route. See GET."},
	"PATCH":      {Sig: "(PATCH pattern [params] body...)", Doc: "Routing DSL: a PATCH route. See GET."},
	"route-form": {Sig: "(route-form verb-fn pattern [params] body...)", Doc: "Routing DSL workhorse: build a typed handler for any verb function (web/get, web/head, …), binding [params] from path params. The verb sugar (GET/POST/…) delegates here."},
	"defroutes":  {Sig: "(defroutes name :middleware [mw...]? route-form...)", Doc: "Routing DSL: define name as a web/Handler from GET/POST/… route forms. The optional leading :middleware [..] clause wraps them via web/wrap (outermost-first). Expands to (def name web/Handler (web/wrap (web/routes ...) mw...))."},
	...
	"web/html-response":  {Sig: "(web/html-response status body)            →  Response", Doc: "Build a text/html response with the given status and (pre-rendered) HTML string body."},
	"web/text-response":  {Sig: "(web/text-response status body)            →  Response", Doc: "Build a text/plain response with the given status and string body."},
	"web/redirect":       {Sig: "(web/redirect location)                    →  Response", Doc: "Build a 302 redirect response with a Location header pointing at location."},
	...
	"web/wrap-session":   {Sig: "(web/wrap-session opts)                    →  Middleware", Doc: "Session middleware backed by an HMAC-signed cookie. Decodes the incoming cookie into req[\"session\"] and signs resp[\"session\"] back out. opts: :secret (or SESSION_SECRET env), :name (\"session\"), :max-age (0 = session cookie), :path (\"/\"), :secure, :http-only (true), :same-site (\"Lax\"/\"Strict\"/\"None\")."},
	"web/session":        {Sig: "(web/session req)                          →  map[string]any", Doc: "Return the request's decoded session map (empty if none, or if wrap-session is not installed)."},
	"web/put-session":    {Sig: "(web/put-session resp session)             →  Response", Doc: "Set resp's outgoing session map (replacing it); wrap-session signs it into the response cookie. Returns resp."},
	"web/assoc-session":  {Sig: "(web/assoc-session resp key value)         →  Response", Doc: "Set key=value in resp's outgoing session, accumulating across calls. Starts from any session staged on resp — to extend the current one, stage it first via (web/put-session resp (web/session req)). Returns resp."},
	"web/clear-session":  {Sig: "(web/clear-session resp)                   →  Response", Doc: "Mark resp to delete the session cookie (logout). Returns resp."},
	...
	"web/htmx-js":         {Sig: "(web/get \"/htmx.js\" web/htmx-js)", Doc: "Handler serving the embedded htmx.min.js (v2.0.4) — no CDN needed; reference it with [:script {:src \"/htmx.js\"}]."},
	"web/hx-request?":     {Sig: "(web/hx-request? req)                      →  bool", Doc: "True when the request was issued by htmx (HX-Request header) — lets one route serve both full pages and fragments."},
	"web/hx-trigger":      {Sig: "(web/hx-trigger resp event)                →  Response", Doc: "Add an HX-Trigger header so htmx fires the named client-side event when the response lands. Multiple calls accumulate."},
	"web/hx-redirect":     {Sig: "(web/hx-redirect url)                      →  Response", Doc: "Response that makes htmx do a client-side redirect to url (200 + HX-Redirect header — htmx ignores it on 3xx)."},
	"web/hx-refresh":      {Sig: "(web/hx-refresh)                           →  Response", Doc: "Response that makes htmx do a full page refresh (HX-Refresh header)."},
	...
	"transduce-json":  {Sig: "(transduce-json xform rf init path) →  [any error]", Doc: "Stream a top-level JSON array's elements (one at a time, constant memory) through transducer xform and reducing fn rf; take/take-while stop reading early. Returns (result, error) — use with if-err."},
