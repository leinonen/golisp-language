# glisp Roadmap — Archive (v1: "Clojure-shaped Go")

> **This is the historical roadmap.** It tracked golisp from a bare transpiler
> through Phases 2–12 to a broad, capable language under the *original*
> identity: "write Clojure-shaped code, ship a single static Go binary"
> (ADR-012). Almost every item here is **done** — collections, strings, the web
> stack, concurrency sugar, gradual typing, the LSP, release infra, and the
> Phase 11/12 work that absorbed the `any` seam and added toolchain-driven typed
> Go interop.
>
> In June 2026 the project re-founded itself around a new identity — **a
> general-purpose Lisp hosted on Go**, the way Clojure is hosted on the JVM
> (ADR-016) — with a compile-time macro system (ADR-017). The live plan now
> lives in [`ROADMAP.md`](ROADMAP.md). This file is preserved unchanged as the
> record of how v1 was built, and as the backlog of any v1 items still marked
> `[ ]` that remain relevant.

**Original goal**: a Lisp-syntax language that transpiles to Go, suited for professional server applications, data transformation, and concurrent systems. Clojure-inspired, not Clojure-compatible. See `docs/adr/` for design decisions.
